...
class GetoptLong
  ...
  def get
    ...
    if argument =~ /^(--[^=]+)/ && @rest_singles.length == 0
      ...
      #
      pattern = $1
      if @canonical_names.include?(pattern) or not self.accept_all_args.nil?
        option_name = pattern
      else
        #
      ...
      #
      if not self.accept_all_args.nil?
        @argument_flags[option_name] = self.accept_all_args
      end
